...
import com.sharkycake.infrastructure.api.aliyunai.model.GetOutPaintingTaskResponse;
import com.sharkycake.infrastructure.api.imagesearch.ImageSearchApiFacade;
import com.sharkycake.infrastructure.api.imagesearch.model.ImageSearchResult;
...
import com.sharkycake.space.auth.annotation.SaSpaceCheckPermission;
...
import com.sharkycake.picture.dto.PictureUpdateRequest;
import com.sharkycake.picture.dto.PictureUploadByBatchRequest;
import com.sharkycake.picture.dto.PictureUploadRequest;
import com.sharkycake.picture.dto.SearchPictureByColorRequest;
import com.sharkycake.picture.dto.SearchPictureByPictureRequest;
import com.sharkycake.picture.service.PictureCleanupTaskService;
import com.sharkycake.picture.service.PictureDeleteService;
...
public class PictureController {
    ...
    /**
     * 上传文件或替换原图。
     * 需要 picture:upload。multipart/form-data 必填 file，文件不超过20MiB，支持jpg/jpeg/png/gif/bmp；可选 id、spaceId、picName、category、tags。新图省略分类时为“未分类”；替换时省略分类/标签则保留原值。指定空间还须所有者，替换还须上传者或平台管理员。
     */
    @ApiOperation(value = "上传文件或替换原图",
            notes = "需要 picture:upload。multipart/form-data 必填 file，文件不超过20MiB，支持jpg/jpeg/png/gif/bmp；可选 id、spaceId、picName、category、tags。新图省略分类时为“未分类”；替换时省略分类/标签则保留原值。指定空间还须所有者，替换还须上传者或平台管理员。")
    @PostMapping("/upload")
    ...
    /**
     * 通过 URL 上传或替换图片。
     * 需要 picture:upload。JSON 必填非空 HTTP(S) fileUrl；可选 id、spaceId、picName、category、tags。新图省略分类时为“未分类”；替换时省略分类/标签则保留原值。指定空间还须所有者，替换还须上传者或平台管理员，spaceId不可跨空间变更。
     */
    @ApiOperation(value = "通过 URL 上传或替换图片",
            notes = "需要 picture:upload。JSON 必填非空 HTTP(S) fileUrl；可选 id、spaceId、picName、category、tags。新图省略分类时为“未分类”；替换时省略分类/标签则保留原值。指定空间还须所有者，替换还须上传者或平台管理员，spaceId不可跨空间变更。")
