...
import com.sharkycake.space.mapper.SpaceUserMapper;
...
public class SpaceUserServiceImpl extends ServiceImpl<SpaceUserMapper, SpaceUser>
    ...
    @Override
    public void leaveTeam(Long spaceId, User loginUser) {
        ThrowUtils.throwIf(spaceId == null || spaceId <= 0, ErrorCode.PARAMS_ERROR, "spaceId必须大于0");
        ...
        ThrowUtils.throwIf(Objects.equals(space.getUserId(), loginUser.getId()),
                ErrorCode.NO_AUTH_ERROR, "空间创建者不能退出团队");
        // 条件同时包含登录用户与空间，避免客户端传入其他成员的 id 后越权删除。
        boolean removed = remove(new QueryWrapper<SpaceUser>()
                .eq("spaceId", spaceId).eq("userId", loginUser.getId()));
        ThrowUtils.throwIf(!removed, ErrorCode.NOT_FOUND_ERROR, "当前用户未加入该团队");
    }
    ...
    public long addSpaceUser(SpaceUserAddRequest spaceUserAddRequest) {
        ...
        try {
            ...
            throw new BusinessException(ErrorCode.OPERATION_ERROR, "该用户已加入团队");
        }
    ...
    public void validSpaceUser(SpaceUser spaceUser, boolean add) {
        ...
        if (add) {
            ...
            ThrowUtils.throwIf(space == null, ErrorCode.NOT_FOUND_ERROR, "空间不存在");
            ThrowUtils.throwIf(!Objects.equals(space.getSpaceType(), SpaceTypeEnum.TEAM.getValue()),
                    ErrorCode.PARAMS_ERROR, "仅团队空间可以添加成员");
            ThrowUtils.throwIf(count(new QueryWrapper<SpaceUser>().eq("spaceId", spaceId).eq("userId", userId)) > 0,
                    ErrorCode.OPERATION_ERROR, "该用户已加入团队");
        }
        // 校验空间角色
    ...
    public List<SpaceUserVO> getSpaceUserVOList(List<SpaceUser> spaceUserList,HttpServletRequest request) {
        ...
        spaceUserVOList.forEach(spaceUserVO -> {
            ...
            spaceUserVO.setUser(userService.convert2UserVO(user));
            // 填充空间信息
            Space space = null;
