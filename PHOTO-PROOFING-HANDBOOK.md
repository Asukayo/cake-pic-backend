...
| 3. 分享与客户选片 | 8～12 小时 | 客户能访问指定单并在上限内选片 | 员工生成/轮换和撤销链接的 Controller、Service 已写；项目归属和管理权限已校验。客户会话及选片尚未实现；固定 7 天与过期时间输入契约尚待对齐 | 仅静态检查，未编译或连接 Redis 验证 |
| 4. 批注与固定清单 | 8～12 小时 | 客户确认，摄影师看到固定修图清单 | 未开始 | 未验证 |
| 5. 成片与交付包 | 12～20 小时 | 成片上传、打包、发布与客户下载 | 未开始 | 未验证 |

**当前下一步：**按用户决定继续第 3 阶段。分享状态只使用 Redis，不增加项目表分享字段。员工生成/轮换及撤销接口代码已写；下一小步实现客户用 `publicId + shareToken` 换取只绑定一个项目的 Redis 会话。固定 7 天与手册约定的可选过期时间仍需在阶段 3 完成前对齐；代码未验证。第二阶段的真实数据库/COS 上传、授权看图、草稿移除、发布、私有桶匿名拒绝、失败清理和并发边界仍待验收；员工页面未确认，不把代码已写记为阶段验收通过。[第二阶段流程图](PROOFING-STAGE2-FLOW.md)可用于复习。
